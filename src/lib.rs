...
mod tests {
    ...
    fn with_shape_should_succeed() {
        ...
        assert!(sess.init_tensor_var(TensorData::I32(vec![2]), vec![0]).is_ok());
        assert!(sess.init_tensor_var(TensorData::F32(vec![2.1]), vec![1]).is_ok());
        assert!(sess.init_tensor_var(TensorData::I32(vec![2, 2]), vec![2]).is_ok());
        assert!(sess.init_tensor_var(TensorData::I32(vec![2, 2]), vec![2, 1]).is_ok());
    }

    #[test]
    fn with_shape_should_fail() {
        let sess = Session::new();
        assert!(sess.init_tensor_var(TensorData::I32(vec![1, 2]), vec![3]).is_err());
    }
